...
function addImportToTestBed(source, specPath, symbolName) {
    ...
    var configureTestingModuleObjectLiterals = allCalls
        ...
        .map(function (c) { return (c.arguments[0].kind === ts.SyntaxKind.ObjectLiteralExpression ? c.arguments[0] : null); });
    if (configureTestingModuleObjectLiterals.length > 0) {
        var startPosition = configureTestingModuleObjectLiterals[0].getFirstToken(source).getEnd();
        return [new change_1.InsertChange(specPath, startPosition, "imports: [" + symbolName + "], ")];
    }
    else {
        return [];
    }
...
function addReexport(source, modulePath, reexportedFileName, token) {
    ...
    if (allExports.length > 0) {
        var m = allExports.filter(function (e) { return e.moduleSpecifier.getText(source).indexOf(reexportedFileName) > -1; });
        if (m.length > 0) {
            var mm = m[0];
            return [new change_1.InsertChange(modulePath, mm.exportClause.end - 1, ", " + token + " ")];
        }
    }
    return [];
}
...
function addGlobal(source, modulePath, statement) {
    ...
    if (allImports.length > 0) {
        ...
        return [new change_1.InsertChange(modulePath, lastImport.end + 1, "\n" + statement + "\n")];
    }
    else {
